...
impl Location {
    ...
    fn new(x: f64, y: f64, z: f64, area: f64, snow: impl Into<Snowball>) -> Self{
...
pub fn find_best_location(locations: Vec<Location>) -> Result<Location, Box<dyn Error>> {
    ...
    locations.into_iter()
        .max_by(|a, b|{
            a.density()
                    .partial_cmp(&b.density()).unwrap_or(std::cmp::Ordering::Equal)

        }).ok_or("No densities found in the given vector".into())
